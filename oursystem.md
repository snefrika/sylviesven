# Our System — Sylvie & Sven

---

## 1. Opening 1♣

Shows a natural 1♣ opening or a balanced hand. The balanced hand is 11-14 or (17)18-19 HCP. It can contain a 5-card diamond suit.

### Responses

| Bid | Meaning |
|-----|---------|
| 1♦ | 4+ hearts, 3+ HCP |
| 1♥ | 4+ spades, 3+ HCP |
| 1♠ | No 4-card major OR balanced OR diamonds, 3+ HCP |
| 1NT | 11-12 HCP, (semi) balanced, 4-card major possible |
| 2♣ | 5+ clubs, INV+ |
| 2♦ | 6♥ weak or strong |
| 2♥ | 6♠ weak or strong |
| 2♠ |  |
| 2NT | 5+ clubs, 0-5 HCP: to play 3♣, even vs 18-19 BAL |
| 3♣ | 5+ clubs, 5-9 HCP: to play 3♣ vs 11-14 BAL, but game vs 18-19 BAL |
| 3♦♥♠ | 7-card, 3-8 HCP |

---

### Sequence 1♣-1♦♥ 

- Transfer accept shows exact 3-card (bad 4333 possible).
- Transfer accept with single jump (2♥ or 2♠) is 4-card support: 11-14 (semi)BAL or bad unbal hand.
- Transfer accept with double jump (3♥ or 3♠) is unbal, not minimum but doesn't show much extras.
- 2NT is (17)18-19 BAL, can still have 3-card support (or 4 spades).
- 3NT is (17)18-19 BAL with 4-card support.
- 1NT is 11-14 BAL OR 1(34)5, denies 3-card support, can still have 4 spades.
- 1♠ shows 4 spades and always unbalanced, so 5+ clubs (or 4144).
- 2♣/3♣ are standard but deny 3-card support.
- Reverses are standard and show (15)16+.
- Jump reverses:
    - 1♣-1♥, 3♦: Strong unbal raise, not GF.
    - 1♣-1♥, 3♥: SPlinter
    - 1♣-1♦, 3♦: Strong unbal raise, not GF.
- 4♣: Strong 6-card and 4-card fit, about 4 losers. (E.G. AQ9x,xx,x,AKQxxx)
- 4M: 6-4 but weaker than 4♣

---

### Sequence 1♣-1♠

Responder has denied a 4-card major unless GF, with diamonds. (If only INV with diamonds and a 4-card major, transfer to your major first.)

| Bid | Meaning |
|-----|---------|
| 1NT | 11-14 BAL |
| 2♣ | Clubs, 11-15 (2♦ is now F1; if responder rebids 2♦/2♠ it shows GF with 5+♦ and 4 in the major) |
| 2♦ | Natural, 11-15. 4♦ and 5+ clubs. Without 4♦, responder has 5+ clubs. 9-card club fit makes 3-level safe. (Also OPPS have at least a 9-card major fit.) |
| 2M | Reverse, could be 3-card with (31)45 |

#### Sequence 1♣-1♠, 2♦

| Bid | Meaning |
|-----|---------|
| Pass | Diamonds |
| 2M | Strong, with diamond fit |
| 2NT | Limit, very rare |
| 3♣ | To play |



#### 1♣-1♠, 1NT

| Bid | Meaning |
|-----|---------|
| 2♣ | INV or weak diamonds |
| 2♦ | GF |
| 2♥/2♠ | Natural 4+ and diamonds, GF |
| 2NT | Weak both minors |
| 3♥/3♠ | Short, (31)45-ish |

---

### The 1NT 11-12 BAL response

| Bid | Meaning |
|-----|---------|
| Pass | 11-12(13), minimum |
| 2♣ | 11-12(13), minimum with clubs |
| 2♥ | Natural, unbalanced, GF |
| 2♠ | Natural, unbalanced, GF |
| 2NT | Balanced with 4-card major (or both), GF |
| 3♦/3♥/3♠ | Shortness (jump = singleton/void), GF |

If opponents intervene: double is takeout.

---

### The 2♣ response (clubs, INV+)

Opener rebids 2NT (11-12 BAL) or 3♣ with minimum hands; others are GF.

---

## 2. Opening 1NT

### Hand type
15–17 HCP, balanced.

### Responses to 1NT

| Bid | Meaning |
|-----|---------|
| 2♣ | Stayman (at least one 4-card major) |
| 2♦ | Transfer to hearts |
| 2♥ | Transfer to spades |
| 2♠ | Range ask / clubs |
| 2NT | Diamonds or both minors |
| 3♣ | Puppet stayman |
| 3♦ | 5-5 majors, INV+ |
| 3♥♠ | Short, (31)(45) |
| 3NT | To play |
| 4♣♦ | Transfer to hearts/spades (also after intervention if bid with jump) |
| 4♥♠ | To play |

---

### 1NT — Stayman (2♣)

Stayman shows at least one 4-card major (or longer), **always**!
- Invitational hands with 5 spades: bid Stayman then 2♠.
- 5-4 in majors: use Stayman. (But weak with 5 spades and 4 hearts: just transfer to spades.)
- If only interested in game: use 3♣ Muppet stayman.
- Transfers after transfers for the majors, but not after Stayman. (2NT invite is needed after Stayman.)
- 4♣♦ = transfer to 4M, non-slammish (even after intervention if bid with jump).
- 4♥♠ = to play, something to protect.

Responses to Stayman: **2♦**, **2♥**, **2♠**.

#### Sequence 1NT-2♣, 2♦

| Bid | Meaning |
|-----|---------|
| Pass | Weak, 5+ diamonds (and at least 34 in majors) |
| 2♥ | Weak with (4)5 hearts and 4 spades |
| 2♠ | INV with 5+ spades |
| 2NT | INV, at least one 4-card major |
| 3♣/3♦ | Natural 5+, GF (no transfers after Stayman) |
| 3♥/3♠ | Smolen: shows 4 and 5+ in other major |

**Smolen:** 1NT-2♣, 2♦-3♠ → 3♠ shows 4 spades and 5+ hearts.
**Smolen:** 1NT-2♣, 2♦-3♥ → 3♥ shows 5+ spades and 4 hearts. (With 5-5 bid direct 3♦ over 1NT.)

#### Sequence 1NT-2♣, 2♥/2♠

| Bid | Meaning |
|-----|---------|
| 2♠ | INV with 5+ spades |
| 2NT | INV, at least one 4-card major (over 2♥ shows 4 spades) |
| 3♣/3♦ | Natural 5+, GF (and shows 4 in other major) |
| Raise | Raises are raises |
| 3OM | Fit, slammish |
| 3NT | Shows 4 cards in other major |
| 4♣/4♦ | Splinter |

#### If opponents double 2♣

Redouble = suggestion to play. 2♦♥♠ are standard responses but show a club stopper. Without club stopper: **Pass**.

**After Pass** (sequence 1NT-(Pass)-2♣-(X), Pass):
Responder bids 2♦ and XX with the weak Stayman hands (2♦ shows diamonds and the majors; XX is weak with both majors). 2♥ and 2♠ are natural and forcing! Lim+, 2♠ can be 5-card.

**After XX** (1NT-(Pass)-2♣-(X), XX-(Pass)):

| Bid | Meaning |
|-----|---------|
| Pass | Some clubs and some points |
| 2♦ | Weak three suiter |
| 2♥ | Weak majors (not forcing!) |
| 2♠ | Natural and forcing INV+, can be 5-card |
| 2NT | INV with at least one major (hearts in this case) |

---

### 1NT — Transfer to hearts (1NT-2♦)

Opener usually bids 2♥. Now 2♠ is invitational with 5+ hearts!

| Bid | Meaning |
|-----|---------|
| Pass | Weak with hearts |
| 2♠ | INV with 5+ hearts (if opener rebids 2NT minimum, 3m is to play with 5♥-5m) |
| 2NT/3♣ | Transfer to a 4+ card minor |
| 3♦ | Transfer, 6+ hearts, INV+ |
| 3♥ | Transfer, 5+ hearts and 4 spades, slammish |
| 3♠ | Transfer to 3NT, asks for fit |
| 3NT | COG (choice of games) |
| 4♣/4♦ | Splinter, 6+ hearts, slammish |
| 4♥ | "I forgot to bid 4♣" |

**The 2♠ INV hand rebid** (1NT-2♦, 2♥-2♠): Showing 5 hearts and invitational strength.
Opener rebids 2NT with minimum and no heart fit. He can rebid 3♥/4♥ with fit and min/max. With max and no 3-card hearts opener can bid something else, usually 3NT.
Over the 2NT rebid, responder can bid 3m to play 5-5. (And can rebid 3♥ with 5 hearts and 4 spades invitational.)

#### If OPPS double (1NT-(Pass)-2♦-(X))

Pass denies three card support. (Now XX is retransfer, 2♥ weak to play.)
XX and 2♥ show support. 2♥ also shows diamond stopper. XX does **not** show playable contract but support!!!

---

### 1NT — Transfer to spades (1NT-2♥)

Never done with an invitational hand and only 5 spades — use Stayman with 5 spades and invitational hand.

| Bid | Meaning |
|-----|---------|
| Pass | Weak with spades |
| 2NT/3♣ | Transfer to 4+ card minor |
| 3♦ | Transfer, 4 hearts (and 5+ spades), slammish |
| 3♥ | Transfer, 6 spades, INV+ |
| 3♠ | Transfer to 3NT, asks for fit |
| 3NT | To play |
| 4♣/4♦ | Splinter, slammish |
| 4♥ | Splinter! Slammish (with 5-5 or longer bid 3♦) |

#### Transfer over Transfer for a minor

E.g. 1NT-2♥, 2♠-3♣: shows 5 spades and 4+ diamonds. Still looking for the best game but could be slammish. Opener can bid 3M or on the four level with fit in the major. Opener can rebid 3OM with 5-card. Opener can rebid 3NT with double stoppers in both other suits (and no great fit for responder). Opener accepts the transfer with usually a fit, but could be a three card if he doesn't stop the other two suits. 

[WIP] After accepting, responder shows his distribution.

**Accepting the double transfer** [WIP]  (1NT-2♦, 2♥-3♣, 3♦):
- **3♥** [WIP]  — 1st step: High shortness: 1543, 0553, 1552
- **3♠** [WIP]  — 2nd step: Low shortness: 3541, 3550, 2551
- **3NT** [WIP]  — 3rd step: 2542
- **4♣(4om)** [WIP]  — 64 hand, very strong
- **4♦(4m)** [WIP]  — Natural 56 ish
- **4♥** [WIP]  — 64 hand, mildly slammish

#### Transfer over Transfer to other major
Shows a slammish 5+ – 4.

#### Transfer over Transfer to same major
Shows 6+ INV or slammish. (Just game forcing: bid direct 4M or 4m.)

---

### 1NT-2♠: Range ask / clubs

Either a hand with long clubs (weak or GF) or a balanced hand (either INV or slammish).
- Opener bids **2NT** if minimal, **3♣** when maximum. When in doubt look at the club holding.
- If responder bids a suit on the three level: shows long clubs and shortage. (3♣ over 2NT is to play, weak.)
- 4NT by responder asks for 4-card suits to play 6 in suit instead of NT.

**Range ask got doubled** (1NT-(Pass)-2♠-(X)):
2NT and 3♣ additionally show a stopper in spades. XX suggests a playable contract. Pass denies a spade stopper, over which partner can redouble to reask, or bid 3♣ (to play) or higher as before.

---

### 1NT-2NT: Transfer to diamonds or both minors

Either weak or GF. If diamonds and GF, always with shortness (or very slammish — you will need to avoid 4♦). If with both minors you cannot stop in 3NT!

Opener rebids his better minor. 1NT-2NT, 3m, and now responder can:

| Bid | Meaning |
|-----|---------|
| Pass | To play |
| 3♦ | Weak with diamonds |
| 3♥/3♠ | Shortness and diamonds |

**Sequence with both minors and GF:** 1NT-2NT, 3m-4m. Now 4NT is sign off.

**Sequence 1NT-2NT, 3♦-3NT:** Responder is showing 3361-ish. As opener diamonds are better than clubs, opener will rarely pass.

**After responder has shown single:** If opener now bids 4M (not in single) it is natural showing a 5-card.

4m support shows 5-5 forcing (4♦ could be with diamonds). 4♦ over 3♣: Strong with diamonds.

---

### 1NT-3♣: Puppet stayman

| Bid | Meaning |
|-----|---------|
| 3♦ | At least one 4-card major |
| 3♥ | 5-card hearts |
| 3♠ | 5-card spades |
| 3NT | No 4-card major |

---

### 1NT-3♦: 5-5 majors, INV+

| Bid | Meaning |
|-----|---------|
| 3♥/3♠ | To play vs INV |
| 3NT | MAX, 2-2 in majors |
| 4♥/4♠ | No much slam interest |
| 4♣/4♦ | Very good hand for ♥/♠ (next step is retransfer, not Last Train) |

---

### What to do with majors

#### Game forcing, not slammish
| Hand | Bid |
|------|-----|
| 6-card major | Bid 4♣♦ |
| 5-card major | Transfer |
| 5-5 majors | 3♦ |
| 5-4 majors | Stayman/Smolen |
| 4-card major | Stayman |

#### Invitational
| Hand | Bid |
|------|-----|
| 6-card major | Transfer and again transfer |
| 5-card ♠ | Stayman and 2♠ |
| 5-card ♥ | Transfer and 2♠ |
| 5-5 majors | 3♦ |
| 5-4 majors | With 5♠4♥: Stayman, over 2♦ rebid 2♠. With 5♥4♠: transfer and show INV and 5♥ (not always 4♠) or GF 4♠5♥ via Stayman and Smolen. |
| 4-card major | Stayman and 2NT (or 3M) |

#### Slammish
| Hand | Bid |
|------|-----|
| 6-card | Transfer and again transfer or splinter |
| 5-card | Transfer and again transfer to 4+ card minor |
| 5-5 majors | 3♦ |
| 5-4 majors | Transfer and again transfer to the 4-card major |
| 4-card major | Stayman and 3OM when fit |
| Balanced | 2♠ range ask |

---

## 3. Opening 2♣

This is either a weak 2♦ opening, or a strong hand. The strong hands are 4-5 losers, GF hands: two-suiters with 5-card major or a strong NT hand: 22-23 or 26-27.

**Responses:**

| Bid | Meaning |
|-----|---------|
| 2♦ | Most hands (waiting) |
| 2♥/2♠ | INV vs weak 2♦, 5+ card, not forcing |
| 2NT | INV+ relay |
| 3♦ | PRE, but prepared for the strong hands |

### Over the 2♦ waiting bid

| Bid | Meaning |
|-----|---------|
| 2♥ | Hearts and another GF, or NT 26-27; forces 2♠ (see below) |
| 2♠ | 5+ card, 3(4) losers (2NT asks for 2nd suit) |
| 2NT | 22-23 BAL |
| 3♣/3♦ | Natural, 6+ card |
| 3♥/3♠ | 6-card, 4(5) losers |
| 3NT | ? |
| 4♣/4♦ | 3-card in 5530, 3 losers |

After 2♥ (forces 2♠ relay):

| Bid | Meaning |
|-----|---------|
| 2NT | 26-27 |
| 3♣ | Natural, 5 hearts and (4)5+ clubs, GF |
| 3♦ | Natural, 5 hearts and (4)5+ diamonds, GF |
| 3♥ | 6+ hearts, bad suit (with 6♥ GF open 2♦) |
| 3♠ | 5+ hearts, 4+ spades, GF |
| 3NT | ? |
| 4♣/4♦ | ?6♥4m, not 5-5 (rebid 3m) |

### Over the 2NT relay

Opener is weak; if not we play 6 or 7.

| Bid | Meaning |
|-----|---------|
| 3♣ | Weak; 3♦ is to play, others GF |
| 3♦ | Not so weak, no 3-card major with honor |
| 3♥/3♠ | Not so weak, 3-card with honor (or any 4-card) |
| 3NT | Good suit |

---

## 4. Opening 2♦

Multi

---

## 5. Intervention after 1m

(We open 1♣ or 1♦, they intervene.)

If there is room (intervention up to 1♥), we employ transfers. Even 1NT = TRF to clubs. TRF to opp's suit not defined. 2♠ is lim+ riase

### One-level overcalls over 1 minor

#### 1♣-(1♦)
| Bid | Meaning |
|-----|---------|
| X | TRF, 4-5 hearts |
| 1♥ | TRF, 4-5 spades |
| 1♠ | TRF to 1NT (XYZ) |
| 1NT | TRF clubs raise |
| 2♣ |TRF ?|
| 2♦♥ | TRF, 6-card |
| 2♠ | INV+, fit (with GF bid 1NT?) |
| 3♦ | [WIP] 5-5M, INV |

#### 1♣-(1♥)
| Bid | Meaning |
|-----|---------|
| X | TRF, 4-5 spades |
| 1♠ | TRF to 1NT |
| 1NT | TRF club raise |
| 2♣ | TRF|
| 2♦ | ?|
| 2♥ | TRF, 6-card |
| 2♠ | INV+, fit |

#### 1♦-(1♥)
| Bid | Meaning |
|-----|---------|
| X | TRF, 4-5 spades |
| 1♠ | TRF to 1NT |
| 1NT |  TRF clubs raise|
| 2♣ | TRF diamonds |
| 2♦ | ? |
| 2♥ | TRF, 6-card |
| 2♠ | INV+, fit |

#### 1m-(1♠)
No transfers, new suit F1 (not gameforcing!), X negative double.

#### 1m-(X) — Transfer system applies

**1♣-(X):**

| Bid | Meaning |
|-----|---------|
| XX | TRF, diamonds |
| 1♦♥ | TRF, 4-5 card |
| 1♠ | TRF to 1NT, could be strong diamonds, D2 |
| 1NT | TRF club raise|
| 2♣ | TRF diamonds|
| 2♦♥ | TRF, 6-card |
| 2♠ | INV+, fit |

**1♦-(X):**

| Bid | Meaning |
|-----|---------|
| XX | TRF, hearts, 4-5 |
| 1♥ | TRF, 4-5 |
| 1♠ | TRF to 1NT |
| 1NT | TRF to clubs |
| 2♣ | TRF riase |
| 2♦♥ | TRF, 6-card |
| 2♠ | INV, fit, diamonds |

---

### [WIP] Sequence 1♦-(2♣)

- [WIP] **Dble** — Can be: (1) 5-10 support; (2) negative: 4-4 in majors, 6+; (3) GF
- [WIP] **2♦♥** — TRF, 5+ card, 8+
- [WIP] **2♠** — INV+fit
- [WIP] **2NT** — Nat, INV
- [WIP] **3♣** — 8+, at least 5-5 in majors

[WIP] **After 1♦-(2♣)-Dble-(Pass)** — Opener rebids:
- [WIP] **2♦** — Min
- [WIP] **3♦** — 13-15, long diamonds
- [WIP] **Other** — 16+. (16+ are not GF, responder can be weak)

[WIP] After the min 2♦ rebid 1♦-(2♣)-Dble-(Pass); 2♦-(Pass)-:
- [WIP] **2♥** — At least 4-4M, 6-9
- [WIP] **2♠** — 4-4M, 10+, F1. (With 54, responder could have done a TRF first)
- [WIP] **2NT+** — GF

[WIP] **After 1♦-(2♣)-Dble-(3♣)** — Dble is takeout. And now:
- [WIP] **3♦** — NF support
- [WIP] **3♥** — 4-4 majors NF
- [WIP] **3♠** — 4-4 majors, 11+, GF

---

### Over 1M (NEW)


#### 1M-(Dble)
Over Dble, TRF. Even 1♥-(Dble)-Rdble is spades.
1♥-(Dble)-2♦: good raise.

#### [WIP] 1M-(2x)
Over (2x), 2NT is INV++ raise and we are in a forcing pass situation.
3x = INV raise.

**[WIP] 1♠-(2♥):**
| Bid | Meaning |
|-----|---------|
| Dble | Both minors, does not show doubleton spade |
| 2♠ | Wide range |
| 2NT | INV+ raise. If lim usually 4-card |
| 3m | GF |
| 3♥ | INV raise |

---

## 6. Conventions

### Good/Bad 2NT

In competitive auctions, a direct 3-level bid shows a **good** (extra values, invitational or forcing) hand, while **2NT is artificial** and shows a **bad** (minimum, just competing) hand. After the 2NT relay, partner bids 3♣ (forced), and the 2NT bidder then places the contract.

**When it applies:** opener only — both sides have bid, RHO has just bid 2 of a suit, and opener wants to bid at the 3-level (new suit or raise of partner's suit).

| Bid | Meaning |
|-----|---------|
| **2NT** (artificial) | Bad hand — minimum, just competing; partner must bid 3♣ relay |
| **Direct 3-level bid** | Good hand — extra values, invitational or game-forcing |

**After the 2NT → 3♣ relay:**

| Continuation | Meaning |
|---|---|
| Pass | Long clubs, competitive |
| 3♦/3♥/3♠ | That suit, competitive (minimum) |
| Raise to partner's suit | Weak/competitive support |
| Direct 3-level raise (no relay) | Good raise, extra values |

**Example:** 1♣-(1♥)-1♠-(2♥), opener holds a minimum with long clubs → bids 2NT (relay), partner bids 3♣, opener passes. With extras → opener bids 3♣ directly.

Related to Lebensohl: same slow=weak / fast=strong principle applied to contested 1-level openings.

---

### XYZ (Double Deux)

General principles after a 1-1-1 sequence:

- All INV hands go over **2♣**
- **2♦** is artificial GF
- **2NT** is transfer to 3♣
- Other bids are weak or GF

#### When XYZ applies

1. Opener has opened on the one level in 1st or 2nd hand. (No XYZ with passed partner.)
2. Responder has bid or doubled.
3. Opener rebids on the one level, or does a support double over a one-level overcall. (Also with 1NT rebid, and RHO passes after opener's rebid.)


